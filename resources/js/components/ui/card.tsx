...
const CardHeader = ({ className, title, description, children, ...props }: HeaderProps) => (
    <div
        slot="header"
        className={cn(
            'grid items-start gap-1.5 p-6 sm:auto-rows-min sm:grid-rows-[auto_auto] sm:has-[[slot=action]]:grid-cols-[1fr_auto]',
            className
        )}
        {...props}
    >
...
const CardDescription = ({ className, ...props }: ComponentPropsWithRef<'div'>) => {
    return <div slot="description" className={cn('text-muted-fg text-sm sm:row-start-2', className)} {...props} />;
};
...
const CardAction = ({ className, ...props }: ComponentPropsWithRef<'div'>) => {
    return (
        <div
            slot="action"
            className={cn(
                'flex items-center gap-2 self-start justify-self-end sm:col-start-2 sm:row-span-2 sm:row-start-1',
                className
            )}
            {...props}
        />
